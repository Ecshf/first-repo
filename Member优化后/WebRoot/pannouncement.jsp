...
<script type="text/javascript">
	function addComment() {
		...
		var success = function(response) {
			$("#content").append("<div style='background-color:#CCCCCC; color:#000000 ;margin:5px'>"+response.username+" 于"+response.commentTime+"发表评论:<br>"+response.content+"</div>");
		}
...
<%
	...
	String sql = "select announcement from announcement WHERE aid="
			+ aid;
...
<body>
	...
	%>
		<div style='background-color:#CCCCCC; color:#000000 ;margin:5px'><%= rs2.getString(1)%> 于<%=commentTime %>发表评论:<br><%=rs2.getString(2)%></div>
	<%} %>
	</div>
	<br>
	<div>发表评论</div>
	...
	<form id="addCommentForm" method="post">
		<table>
			...
			<tr>
				<td colspan="2"><input hidden="hidden" name="username" id="username"
					value="<%=session.getAttribute("adminName")%>"></td>
			</tr>
			<tr>
				<td width="100px"><label>评论内容：</label></td>
				<td><textarea name="contentArea" id="contentArea" rows="10" cols="50" ></textarea></td>
			</tr>
			<tr>
