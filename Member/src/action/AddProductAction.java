	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			mySmartUpload
					.getFiles()
					.getFile(0)
					.saveAs(path + System.currentTimeMillis()
							+ mySmartUpload.getFiles().getFile(0).getFileName());
			...
			bound = Integer.valueOf(mySmartUpload.getRequest().getParameter(
					"bound"));
			// 显示处理结果
			System.out.println(count + "文件已上传至：" + path);

			JsonObject json = ProductDao.add(ProductID, Version, PType,
					Content, bound, filename, addTime);
			writer.print(json);
			writer.flush();
		} catch (Exception e) {
			e.printStackTrace();
